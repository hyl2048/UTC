...
class MLMTokenizerWrap(object):
    ...
    def __call__(self, inputs: List[Dict[str, Any]]):
        ...
        max_lengths = self._create_max_lengths_from_do_truncate(
            orig_input_ids, part_do_truncate
        )
    ...
    def _create_max_lengths_from_do_truncate(
        self, orig_input_ids: List[List[int]], part_do_truncate: List[bool]
    ):
        """
            ...
            if part_do_truncate[index]:
                ...
                if max_length < 0:
                    raise AssertionError(
                        "Actual length has extended the maximum length."
                    )
    ...
    def _create_position_ids_from_part(self, inputs, part, last_position):
        ...
        if self.omask_token in part["text"]:
            ...
            for start_id, end_id in zip(omask_index[:-1], omask_index[1:]):
                position_ids.extend(
                    list(range(last_position, last_position + end_id - start_id))
                )
