...
from lion_optimizer.lion import Lion
...
def create_data_loader(
    ...
    return DataLoader(
        dataset, batch_size=batch_size, num_workers=num_workers, collate_fn=collator
    )
...
def main():
    ...
    fabric = Fabric(
        accelerator="cuda", devices=args.devices, strategy=strategy, loggers=logger
    )
    # fabric.launch()
    fabric.seed_everything(args.seed + fabric.global_rank)
    fabric.print(
        "world_size {}, global_rank {}".format(fabric.world_size, fabric.global_rank)
    )
    ...
    #     betas=(0.95, 0.98),
    # )
    # optimizer = torch.optim.lion(optimizer_grouped_parameters, lr=args.learning_rate, eps=args.adam_epsilon)
    estimated_stepping_batches = math.ceil(
        len(train_loader) / args.grad_accum_steps
    ) * max(args.max_epochs, 1)
    ...
    train_metrics = MetricCollection(
        {"micro_f1": F1Score(**micro_conf), "macro_f1": F1Score(**macro_conf)}
    )
