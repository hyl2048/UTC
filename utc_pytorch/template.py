...
class Template:
    ...
    def __init__(
        self, prompt: str, tokenizer: PreTrainedTokenizer, max_length: int, **kwargs
    ):
    ...
    def create_truncation_sequence_from_prompt(
        self, prompt: Optional[List[Dict[str, Any]]] = None
    ) -> List[bool]:
    ...
    def create_example_keys_from_prompt(self):
        ...
        if len(example_keys) == 0:
            raise ValueError(
                'No `text` keyword in template: "{}", please check it again.'.format(
                    self.prompt
                )
            )
        ...
        for index, part in enumerate(prompt):
            if "text" in part:
                if part["text"] not in example:
                    raise ValueError(
                        "Unexpected value in template.Can not find keyword {} in example:{}".format(
                            part["text"], example
                        )
                    )
    ...
    def parse_template_string(
        prompt: str, left_token: Optional[str] = "{", right_token: Optional[str] = "}"
    ):
        """
...
class UTCTemplate(Template):
    ...
    def __init__(
        self, tokenizer: PreTrainedTokenizer, max_length: int, prompt: str = None
    ):
        prompt = (
            (
        ...
        if not self._has_options():
            raise ValueError(
                "Expected `options` and `add_omask` are in defined prompt, but got {}".format(
                    self.prompt
                )
            )
    ...
    def encode(self, example: Dict[str, Any], use_mask: bool = False):
        ...
        input_dict["omask_positions"] = (
            np.where(np.array(input_dict["input_ids"]) == omask_token_id)[0]
            .squeeze()
            .tolist()
        )

        sep_positions = (
            np.where(np.array(input_dict["input_ids"]) == self.tokenizer.sep_token_id)[
                0
            ]
            .squeeze()
            .tolist()
